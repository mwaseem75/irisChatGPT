...
def handle_userinput(user_question):
    if st.session_state['rb'] == 'Intersystems Objectscript Reference':
        response = st.session_state.conversation_isdoc.invoke({'question': user_question})
        ...
        with st.expander("Conversation", expanded=True):
            for i in range(len(qst)):
                st.write(user_template.replace("{{MSG}}", qst[i]), unsafe_allow_html=True)
                st.write(bot_template.replace("{{MSG}}", ans[i]), unsafe_allow_html=True)

    if st.session_state['rb'] == 'InterSystems Grand Prix Contest':
        response = st.session_state.conversation_iscontest.invoke({'question': user_question})
